...
async function sendFrameToBackendImage() {
    if (!$canvas) return;

    // Lấy snapshot của camera thông qua canvas
    const blob = await new Promise((resolve) => $canvas.toBlob(resolve, 'image/jpeg', 0.8));

    if (!blob) {
        console.warn("Không thể tạo blob từ canvas");
        return;
    }
    ...
    try {
        const response = fetch('http://127.0.0.1:5000/upload-image', {
            method: 'POST',
            body: formData,
        });

        if (response.ok) {
            console.log("Đã gửi ảnh thành công!");
        } else {
            console.error("Lỗi khi gửi ảnh:", response.statusText);
        }
    } catch (err) {
...
// Hàm bắt đầu gửi frame
function startSendingFrames() {
    if (!videoActive) {
        alert("Camera chưa bật. Vui lòng bật camera.");
        return;
    }

    sendInterval = setInterval(() => {
        sendFrameToBackendImage();
    }, 300);

    isSendingFrames = true;
    $btnSendFrame.textContent = 'Stop send image';
    updateVideoBadge();
}
...
function stopSendingFrames() {
    ...
    $btnSendFrame.textContent = 'Send to AI';
    updateVideoBadge();
}

// Sự kiện cho nút "Bắt đầu gửi" hoặc "Ngừng gửi"
$btnSendFrame.addEventListener('click', () => {
    isSendingFrames ? stopSendingFrames() : startSendingFrames();
});
